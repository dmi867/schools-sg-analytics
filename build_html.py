...
<style>
  ...
  * { box-sizing:border-box }
  ...
  table.dense th,table.dense td { padding:8px 8px; vertical-align:middle; overflow:hidden }
  table.dense th { font-size:.7rem; letter-spacing:.01em; white-space:normal; line-height:1.25; vertical-align:middle }
  table.dense td.top { vertical-align:top }
  ...
  table.dense .dates { display:grid; gap:2px; width:100%; max-width:15rem }
  table.dense .exp-line { display:grid; grid-template-columns:5.2em auto; gap:0 8px; align-items:baseline; font-size:.7rem; color:var(--muted); line-height:1.35 }
  table.dense .exp-line b { color:var(--text); font-weight:600; font-variant-numeric:tabular-nums }
  table.dense .exp-line .late { color:#A32E2E }
  table.dense .pill { font-size:.68rem; padding:2px 7px; gap:3px; white-space:nowrap; max-width:100%; overflow:hidden; text-overflow:ellipsis }
  ...
  table.dense .money { font-size:.8rem }
  table.dense .amt { font-weight:600; font-variant-numeric:tabular-nums; font-size:.84rem; line-height:1.2 }
  table.dense .pct { font-size:.72rem; color:var(--muted); line-height:1.2 }
...
DASHBOARD_BODY = r"""
  ...
  <div class="tabpanel" data-tab="objects">
    ...
    <p class="note" style="margin-top:0">Снизу оплата, слева СГ. Диагональ — идеал. Коридор ±10 п.п. Вертикали <strong>30%</strong> и <strong>50%</strong> — типичные авансы. Выше диагонали — кредитует подрядчик, ниже — избыток оплаты.</p>
    ...
    <div class="tbl-wrap" style="max-height:440px">
      <table class="full dense" id="objectsTable">
        <colgroup>
          ...
          <col style="width:9%"><col style="width:20%"><col style="width:11%">
        </colgroup>
        <thead><tr>
          <th>Объект</th>
          <th>Округ</th>
          <th class="r">Контракт, млн ₽</th>
          <th class="r">Готовность, %</th>
          <th class="r">Оплата, %</th>
          <th class="r">Разница, млн ₽</th>
          <th>Статус</th>
          <th>Экспертиза</th>
          <th class="r">После экспертизы</th>
        </tr></thead>
        <tbody id="objTbl"></tbody>
      </table>
    </div>
    <p class="note">В колонке «Экспертиза» — статус и сроки (заход, получение, регламент +42 рабочих дня). «После экспертизы» — согласованная сумма и изменение к плановой. Клик по строке — «Один объект».</p>
  </div>
  ...
  <div class="tabpanel" data-tab="contractors" hidden>
    ...
    <div class="tbl-wrap">
      <table class="full dense" id="contrTable">
        ...
        <thead><tr>
          <th>Подрядчик</th>
          <th class="r">Объектов</th>
          <th class="r" title="Сумма контрактов по всем его объектам, весь срок">Контракт, млн ₽</th>
          <th class="r" title="Лимит финансирования на 2026 год по всем объектам подрядчика">Финансирование 2026, млн ₽</th>
    ...
    <div id="contrDetail" hidden style="margin-top:16px">
      ...
      <div class="tbl-wrap" style="max-height:360px">
        <table class="full dense">
          <colgroup>
            <col style="width:28%"><col style="width:8%"><col style="width:12%">
            <col style="width:16%"><col style="width:12%"><col style="width:12%"><col style="width:12%">
          </colgroup>
          <thead><tr>
            <th>Школа</th>
            <th class="r">Готовность, %</th>
            <th class="r">Контракт, млн ₽</th>
            <th class="r">Финансирование 2026, млн ₽</th>
            <th class="r">Освоение 2026</th>
            <th class="r">Разница, млн ₽</th>
            <th>Статус</th>
          </tr></thead>
...
const STATUS_SHORT = { over:'Избыток', credit:'Кредитует', balanced:'Баланс', unknown:'—' };
...
function renderObjTbl() {
  ...
  document.getElementById('objTbl').innerHTML = rows.map(o => {
    ...
    const overdue = days!=null
      ? (days<0
        ? `<span class="pill err" title="просрочка ${Math.abs(days)} дн.">−${Math.abs(days)} дн.</span>`
        : `<span class="pill ok" title="осталось ${days} дн.">+${days} дн.</span>`)
      : '—';
    let overrun;
    if (o.exp_pending && o.exp_last_result==='Положительное' && o.sd_confirmed) {
      ...
      overrun = '<span class="pill ok" title="Техчасть ок, сметы нет">ТЧ</span>';
    } else {
    ...
    const afterTip = o.exp_pending
      ? 'согласованная смета по прошлому прохождению; сейчас идёт повторная экспертиза'
    ...
    const stack = `<div class="stack" title="${tip}">` +
      ...
      `<span class="sub">${o.contractor||'—'}</span>` +
      `<span class="uin">${o.uin||'—'}</span></div>`;
    return `<tr class="clickable" data-uin="${o.uin}">` +
      `<td class="top">${stack}</td>` +
      ...
      `<td class="top">${statusCell}</td><td class="top">${expCell}</td><td class="r top">${smeta}</td></tr>`;
  }).join('');
...
function renderContrDetail() {
  ...
  document.getElementById('contrDetailSum').textContent =
    `${objs.length} объект(ов) · контракт ${Math.round(contract).toLocaleString('ru-RU')} млн ₽ · финансирование 2026 ${Math.round(plan).toLocaleString('ru-RU')} млн ₽ · освоено ${Math.round(osv).toLocaleString('ru-RU')} · остаток ${Math.round(Math.max(0, plan-osv)).toLocaleString('ru-RU')}`;
    ...
    options: {
      ...
      plugins: {
        ...
        annotation: {
          annotations: {
            ...
            diagCredit: {
              type: 'line', xMin: 0, yMin: 10, xMax: 90, yMax: 100,
              borderColor: 'rgba(147,168,188,.7)', borderWidth: 1, borderDash: [2, 3],
            },
            diagOver: {
              type: 'line', xMin: 10, yMin: 0, xMax: 100, yMax: 90,
              borderColor: 'rgba(147,168,188,.7)', borderWidth: 1, borderDash: [2, 3],
            },
            adv30: {
              type: 'line', xMin: 30, xMax: 30, yMin: 0, yMax: 100,
              borderColor: 'rgba(27,138,156,.55)', borderWidth: 1.5, borderDash: [4, 3],
              label: {
                display: true, content: 'аванс 30%', position: 'start',
                backgroundColor: 'rgba(27,138,156,.9)', color: '#fff',
                font: { size: 10, weight: '600', family: "'Golos Text', Manrope, system-ui" },
                padding: 4,
              },
            },
            adv50: {
              type: 'line', xMin: 50, xMax: 50, yMin: 0, yMax: 100,
              borderColor: 'rgba(20,50,96,.5)', borderWidth: 1.5, borderDash: [4, 3],
              label: {
                display: true, content: 'аванс 50%', position: 'start',
                backgroundColor: 'rgba(20,50,96,.88)', color: '#fff',
                font: { size: 10, weight: '600', family: "'Golos Text', Manrope, system-ui" },
                padding: 4,
              },
            },
      ...
      scales: {
        x: {
          ...
          ticks: {
            color: '#8BA4B8', font: { size: 10 }, stepSize: 10,
            callback: (v) => [0, 20, 30, 40, 50, 60, 80, 100].includes(v) ? v : undefined,
          },
