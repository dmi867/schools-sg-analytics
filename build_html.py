...
<style>
  ...
  * { box-sizing:border-box }
  ...
  table.dense { table-layout:fixed; width:100%; font-size:.74rem }
  table.dense th,table.dense td { padding:5px 5px; vertical-align:middle }
  table.dense th { font-size:.64rem; letter-spacing:.02em; white-space:nowrap }
  table.dense .clip { overflow:hidden; text-overflow:ellipsis; white-space:nowrap }
  table.dense .pill { font-size:.64rem; padding:1px 6px; gap:3px; white-space:nowrap }
  table.dense .pill.ok::before, table.dense .pill.warn::before, table.dense .pill.err::before { width:5px; height:5px }
  table.dense .flag { font-size:.6rem; padding:0 4px; margin:0 0 0 3px }
  table.dense .money { font-size:.74rem }
...
DASHBOARD_BODY = r"""
  ...
  <div class="tabpanel" data-tab="objects">
    ...
    <div class="tbl-wrap fit" style="max-height:520px">
      <table class="full dense" id="objectsTable">
        <colgroup>
          <col style="width:14%"><col style="width:9%"><col style="width:13%">
          <col style="width:6%"><col style="width:5%"><col style="width:5%"><col style="width:7%">
          <col style="width:8%"><col style="width:7%"><col style="width:8%"><col style="width:18%">
        </colgroup>
        <thead><tr>
          <th>Школа</th><th>Округ</th><th>Подрядчик</th>
          ...
          <th class="r" title="Удорожание: % · млн ₽ → согласованная смета">Смета после эксп.</th>
        </tr></thead>
        <tbody id="objTbl"></tbody>
      </table>
    </div>
    <p class="note">Без горизонтальной прокрутки: длинные названия обрезаны (наведите — полное). Сортировка по недоплате подрядчику. «Смета после эксп.»: % удорожания · сумма → согласованная. Клик по строке — «Один объект».</p>
  </div>
...
function renderObjTbl() {
  ...
  document.getElementById('objTbl').innerHTML = rows.map(o => {
    ...
    const tip = [o.full, o.rp ? 'РП: '+o.rp : '', o.contractor||''].filter(Boolean).join(' · ');
    return `<tr class="clickable" data-uin="${o.uin}">` +
      `<td class="clip" title="${tip}">${o.name}${o.advance_stuck?' <span class="flag">аванс</span>':''}</td>` +
      `<td class="clip" title="${o.municipality||''}">${o.municipality||'—'}</td>` +
      `<td class="clip" title="${o.contractor||''}">${o.contractor||'—'}</td>` +
