...
DASHBOARD_BODY = r"""
  ...
  <div class="tabpanel" data-tab="contractors" hidden>
    ...
    <p class="note" style="margin-top:0">Сумма — сколько млн ₽ готовности уже «закрыто» без оплаты (подрядчик строит на свои). Строки по убыванию этой суммы. «Доля нарастающим» — сколько % всего такого риска по портфелю закрывают подрядчики сверху списка. Клик — фильтр объектов по этому подрядчику.</p>
    <div class="tbl-wrap">
      <table class="full">
        <thead><tr>
          <th>Подрядчик</th><th class="r">Объектов</th><th class="r">Контракт, млн ₽</th>
          <th class="r">Кредитует стройку, млн ₽</th><th class="r">Доля нарастающим</th>
...
function renderRollup() {
  ...
  document.getElementById('contrRollup').innerHTML = rows.map(c => {
    cum += c.credit;
    const cumPct = totalCredit > 0 && c.credit > 0 ? Math.round(cum / totalCredit * 1000) / 10 : null;
    const creditTxt = c.credit > 0 ? Math.round(c.credit).toLocaleString('ru-RU') : '—';
    const shareTxt = cumPct != null ? cumPct + '%' : '—';
