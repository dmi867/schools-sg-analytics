...
DASHBOARD_BODY = r"""
  ...
  <div class="tabpanel" data-tab="objects">
    ...
    <p class="note">Таблица по убыванию приоритета. Приоритет = сколько подрядчик кредитует (млн) × срочность (просрочка, 0% бюджета) × вес объекта у этого подрядчика. Клик по строке — вкладка «Один объект». Экспертиза: <span class="pill ok">ТЧ ок</span> / <span class="pill err">ТЧ отклонена</span>, <span class="pill ok">СД ок</span> / <span class="pill err">СД нет</span>, <span class="pill warn">в экспертизе</span> — заявка ещё открыта.</p>
  </div>
...
function renderObjTbl() {
  ...
  document.getElementById('objTbl').innerHTML = rows.map(o => {
    ...
    if (o.exp_last_result==='Отрицательное') {
      overrun = '<span class="pill-pair"><span class="pill err" title="Отрицательное заключение экспертизы">ТЧ отклонена</span></span>';
      ...
      overrun = '<span class="pill-pair"><span class="pill ok" title="Техчасть (ОПД) — положительное заключение">ТЧ ок</span><span class="pill err" title="Смета не согласована: нет согласованной стоимости в Акцент">СД нет</span></span>';
    } else if (o.exp_last_result==='Положительное' && o.sd_confirmed) {
      const u = o.exp_overrun!=null ? (o.exp_overrun>5 ? ` +${o.exp_overrun}%` : '') : '';
      overrun = `<span class="pill-pair"><span class="pill ok" title="Техчасть ок">ТЧ ок</span><span class="pill ok" title="Смета согласована">СД ок${u}</span></span>`;
    } else {
